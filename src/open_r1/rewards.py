...
def accuracy_reward(completions, solution, **kwargs):
    ...
    # print(f"[DEBUG accuracy_reward] Called with {len(completions)} completions")
    # print(f"[DEBUG accuracy_reward] solution type: {type(solution)}, len: {len(solution) if solution else 'None'}")
    # if solution and len(solution) > 0:
    #     first_sol = str(solution[0])
    #     print(f"[DEBUG accuracy_reward] First solution: {first_sol[:100]}..." if len(first_sol) > 100 else f"[DEBUG accuracy_reward] First solution: {first_sol}")

    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    for idx, (content, sol) in enumerate(zip(contents, solution)):
        # print(f"\n[DEBUG accuracy_reward] === Processing completion {idx+1}/{len(contents)} ===")
        sol_str = str(sol)
        # print(f"[DEBUG accuracy_reward] Solution: {sol_str[:100]}..." if len(sol_str) > 100 else f"[DEBUG accuracy_reward] Solution: {sol_str}")
        ...
        # print(f"[DEBUG accuracy_reward] gold_parsed: {gold_parsed}")

        if len(gold_parsed) != 0:
            # We require the answer to be provided in correct latex (no malformed operators)
            # print(f"[DEBUG accuracy_reward] Content sample: {content[:200]}...")
            answer_parsed = parse(
                content,
                extraction_config=[
            ...
            # print(f"[DEBUG accuracy_reward] answer_parsed: {answer_parsed}")

            # Reward 1 if the content is the same as the ground truth, 0 otherwise
            try:
                reward = float(verify(answer_parsed, gold_parsed))
                # print(f"[DEBUG accuracy_reward] Verification successful, reward: {reward}")
            except Exception as e:
                # print(
                #     f"[DEBUG accuracy_reward] verify failed: {e}, answer: {answer_parsed}, gold: {gold_parsed}"
                # )
                reward = 0.0
        else:
            # If the gold solution is not parseable, we reward 1 to skip this example
            reward = 1.0
            # print(f"[DEBUG accuracy_reward] Failed to parse gold solution: {sol}")

        rewards.append(reward)
        # print(f"[DEBUG accuracy_reward] Final reward for completion {idx+1}: {reward}")

    # print(f"[DEBUG accuracy_reward] Returning {len(rewards)} rewards: {rewards[:5]}..." if len(rewards) > 5 else f"[DEBUG accuracy_reward] Returning rewards: {rewards}")
    return rewards
